...
run_ae = True
encoding_dim = 256
layer_sizes = (512, 512, 256, 128, 64)
...
if __name__ == "__main__":
    ...
    GIP_CM = utils.load_pickle(FEATURES_DIR, f"{data_name}_GIP_CM.pkl")
    ...
    all_fs = ["GIP_CD", "GIP_CM", "SIM_CC", "GIP_DC", "GIP_DM", "SIM_DD"]

    all_combinations = []
    for i in range(2, len(all_fs) + 1):
        i_comb = [sorted(comb) for comb in permutations(all_fs, i)]
        for comb in i_comb:
            grouped = {}
            for elem in comb:
                category = elem[4]
                grouped.setdefault(category, [])
                grouped[category].append(elem)
                grouped[category] = sorted(grouped[category])

            if len(list(grouped.keys())) == 2:
                comb = (sorted(grouped["C"]), sorted(grouped["D"]))
                if comb not in all_combinations:
                    all_combinations.append(comb)

    for (
        circrna_feature_matrices_names,
        disease_feature_matrices_names,
    ) in tqdm(all_combinations, position=0, leave=True):
    ...
    pd.DataFrame(RESULTS).to_excel(os.path.join(RESULTS_DIR, "feature_selection.xlsx"))
